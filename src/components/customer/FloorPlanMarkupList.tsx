...
export function FloorPlanMarkupList({
  ...
  const uploadToStorage = async (file: File, markupId: string): Promise<string> => {
    ...
    const {
      error: uploadError
    } = await supabase.storage.from('ticket-markups').upload(filePath, file, {
    ...
    } = supabase.storage.from('ticket-markups').getPublicUrl(filePath);
    return publicUrl;
  };
  ...
  return <div className="space-y-3">
      ...
      {markups.length === 0 ? <Card className="border-dashed">
        ...
        return <Card key={markup.id} className={cn("transition-all cursor-pointer hover:shadow-md", selectedMarkupId === markup.id && "ring-2 ring-primary")} onClick={() => onMarkupSelect(markup.id)}>
                <CardContent className="p-3 space-y-3">
                  <div className="flex items-start gap-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      {markup.type === "pin" ? <MapPin className="h-4 w-4 text-destructive flex-shrink-0" /> : <Square className="h-4 w-4 text-yellow-500 flex-shrink-0" />}
                ...
                onMarkupDelete(markup.id);
                  ...
                  <div onClick={e => e.stopPropagation()}>
                    ...
                    <Input id={`note-${markup.id}`} value={markup.notes || ""} onChange={e => onMarkupUpdate(markup.id, e.target.value)} placeholder="Describe the issue..." className="text-sm h-8 mt-1" />
                  </div>

                  {/* Photo section */}
                  <div className="space-y-2" onClick={e => e.stopPropagation()}>
                    <Label className="text-xs">Photo (optional)</Label>
                    {uploadingPhotos.has(markup.id) ? <div className="flex items-center justify-center h-24 border border-border rounded-lg bg-muted">
                        <div className="flex flex-col items-center gap-2">
                          <Loader2 className="h-4 w-4 animate-spin text-primary" />
                          <span className="text-xs text-muted-foreground">Uploading...</span>
                        </div>
                      </div> : photoPreview ? <div className="relative rounded-lg overflow-hidden border border-border">
                        <img src={photoPreview} alt="Markup photo" className="w-full h-24 object-cover" />
                        <Button size="icon" variant="destructive" className="absolute top-1 right-1 h-6 w-6" onClick={e => {
                  e.stopPropagation();
                  onMarkupPhotoUpdate(markup.id, null);
                }}>
                          <X className="h-3 w-3" />
                        </Button>
                      </div> : <Button variant="outline" size="sm" className="w-full h-8" onClick={e => {
                e.stopPropagation();
