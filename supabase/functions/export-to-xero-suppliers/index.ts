...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Exporting ${suppliers.length} suppliers to Xero`);

    let successCount = 0;
    const errors: any[] = [];

    // Export each supplier to Xero
    for (const supplier of suppliers) {
      try {
        ...
        const xeroContact: any = {
          ...
          ContactStatus: supplier.is_active ? "ACTIVE" : "ARCHIVED",
        };
        ...
        if (!createResponse.ok) {
          ...
          console.error(`Failed to create supplier ${supplier.name}:`, errorData);
          errors.push({
          ...
          successCount++;
          console.log(`Successfully exported supplier: ${supplier.name}`);
        }
      } catch (error) {
        console.error(`Error exporting supplier ${supplier.name}:`, error);
        errors.push({
          supplierId: supplier.id,
          supplierName: supplier.name,
    ...
    console.error("Error exporting suppliers to Xero:", error);
    return new Response(
      JSON.stringify({ 
        success: false,
        error: error instanceof Error ? error.message : String(error) 
