...
serve(async (req) => {
  ...
  try {
    ...
    console.log("📧 Starting archived email sync for account:", emailAccountId);
    ...
    console.log("📬 Fetching archived emails for:", emailAccount.email_address);
    ...
    // Fetch archived emails from Microsoft Graph API
    const messagesResponse = await fetch(
      `https://graph.microsoft.com/v1.0/users/${emailAccount.email_address}/mailFolders/archive/messages?$top=50&$orderby=receivedDateTime desc&$select=id,subject,from,receivedDateTime,bodyPreview,body,isRead,conversationId,hasAttachments&$expand=attachments`,
      {
        headers: {
    ...
    if (!messagesResponse.ok) {
      ...
      console.error("Failed to fetch archived messages:", errorText);
      ...
      await supabase
        ...
        .update({
          sync_status: "error",
          sync_error: `Failed to fetch archived emails: ${errorText}`,
        })
        .eq("id", emailAccountId);
      
      throw new Error("Failed to fetch archived emails from Microsoft");
    }

    const messagesData = await messagesResponse.json();
    const messages = messagesData.value || [];

    console.log(`✅ Found ${messages.length} archived messages`);

    let syncedCount = 0;
    let errorCount = 0;

    // Process each archived email
    for (const message of messages) {
      try {
        ...
        if (existingTicket) {
          ...
          console.log(`⏭️ Updated existing ticket to archived: ${message.subject}`);
          continue;
        }
        ...
        if (ticketError) {
          console.error("Error creating archived ticket:", ticketError);
          errorCount++;
          continue;
        }
        ...
        if (messageError) {
          console.error("Error creating archived message:", messageError);
          errorCount++;
        } else {
          syncedCount++;
          console.log(`✅ Created archived ticket: ${message.subject}`);
        }
      } catch (error) {
        console.error(`Error processing archived message ${message.id}:`, error);
        errorCount++;
      }
    }
    ...
    console.log(`🎉 Archived sync complete: ${syncedCount} new tickets, ${errorCount} errors`);
    ...
    console.error("Error in microsoft-sync-archived-emails:", error);
