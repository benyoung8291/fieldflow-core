...
-- Fix the generate_service_orders_from_contracts function to use BIGINT for order numbers
...
DECLARE
  ...
  v_order_id UUID;
  v_order_number TEXT;
  v_last_number BIGINT;  -- Changed from INT to BIGINT
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      v_last_number := FLOOR(EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT;
      v_order_number := 'SO-' || v_last_number::TEXT;
