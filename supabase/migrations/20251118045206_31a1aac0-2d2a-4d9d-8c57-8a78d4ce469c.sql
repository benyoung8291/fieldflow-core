-- Add detailed logging to generation function
...
BEGIN
  ...
  RAISE NOTICE 'Found % total items in date range % to %', v_total_items, p_start_date, p_end_date;
  ...
  RAISE NOTICE 'Found % already generated items', v_already_generated;
  ...
  LOOP
    BEGIN
      RAISE NOTICE 'Processing group: location=%, date=%, items=%', v_group.location_id, v_group.generation_date, array_length(v_group.line_item_ids, 1);
      
      -- Get and increment sequential number (inline)
      ...
      RAISE NOTICE 'Creating service order: %, title=%, amount=%', v_order_number, v_title, v_fixed_amount;
      ...
      RAISE NOTICE 'Created service order with ID: %', v_order_id;
      ...
      RAISE NOTICE 'Successfully created order % with % line items', v_orders_created, array_length(v_group.line_item_ids, 1);

    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'ERROR creating service order: %', SQLERRM;
  ...
  RAISE NOTICE 'Generation complete: % orders created', v_orders_created;
