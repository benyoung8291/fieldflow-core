-- Update get_user_access_info to explicitly handle users with no roles
-- This is a defense-in-depth security measure

CREATE OR REPLACE FUNCTION public.get_user_access_info()
 RETURNS TABLE(user_id uuid, has_role boolean, is_worker boolean, is_customer boolean, customer_id uuid, can_access_office boolean, can_access_worker boolean, can_access_customer_portal boolean, show_toggle boolean, default_route text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- SECURITY: If user has no roles at all, deny all access explicitly
  IF NOT v_has_role THEN
    -- Check if they're a customer portal user (separate from roles)
    ...
    -- If they're a customer, allow customer portal access only
    IF v_is_customer THEN
      RETURN QUERY SELECT
        ...
        false as can_access_office,
        false as can_access_worker,
        true as can_access_customer_portal,
        false as show_toggle,
        '/customer'::text as default_route;
      RETURN;
    END IF;

    -- No roles and not a customer - deny all access
    RETURN QUERY SELECT
      v_user_id as user_id,
      false as has_role,
      false as is_worker,
