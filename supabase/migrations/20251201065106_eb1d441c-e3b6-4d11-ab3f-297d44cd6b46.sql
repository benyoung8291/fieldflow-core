-- Drop and recreate the function with corrected location_id reference
...
BEGIN
  FOR v_line_item IN
    ...
    ORDER BY sc.contract_number, scli.next_generation_date, scli.line_number
  LOOP
    ...
    WHERE so.tenant_id = p_tenant_id
      ...
      AND so.contract_line_item_id = v_line_item.id
      AND so.preferred_date = v_generation_date
    LIMIT 1;
    ...
    IF v_existing_order_id IS NULL THEN
      ...
      INSERT INTO contract_generation_history (
        tenant_id,
        contract_id,
        line_item_id,
        generation_date,
